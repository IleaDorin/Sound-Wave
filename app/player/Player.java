...
public final class Player {
    ...
    public void setPaused(final boolean pausedStatus) {
    ...
    /**
     * Gets stats.
     * @param reversePaused indicator for reversing the pause field
     * in case a user was playing before going offline
     * @return the stats
     */
    public PlayerStats getStats(final boolean reversePaused) {
        String filename = "";
        int duration = 0;
        if (source != null && source.getAudioFile() != null) {
    ...
    /**
     * Pause that only works one way (play->pause).
     */
    ...
    /**
     * Checks to see if the current source is a specific podcast.
     *
     * @param podcastName podcast to find
     * @return a true or false response
     */
    public boolean isCurrentPodcast(final String podcastName) {
        PlayerSource currentSource = getSource();
        if (currentSource != null && type.equals("podcast")) {
            Podcast currentPodcast = (Podcast) currentSource.getAudioCollection();
            return currentPodcast.getName().equals(podcastName);
        }
        return false;
    }
     ...
     * Checks to see if the current source is a specific podcast.
     *
     * @param albumName album to find
     * @return a true or false response
     */
    public boolean isCurrentAlbum(final String albumName) {
        PlayerSource currentSource = getSource();
        if (currentSource != null && type.equals("album")) {
            Album currentAlbum = (Album) currentSource.getAudioCollection();
            return currentAlbum.getName().equals(albumName);
        }
        return false;
    }

    /**
     * Checks to see if the current source is a specific playlist.
     *
     * @param playlistName playlist to find
     * @return a true or false response
     */
    public boolean isCurrentPlaylist(final String playlistName) {
        PlayerSource currentSource = getSource();
        if (currentSource != null && type.equals("playlist")) {
            Playlist currentPlaylist = (Playlist) currentSource.getAudioCollection();
            return currentPlaylist.getName().equals(playlistName);
        }
        return false;
    }
}
